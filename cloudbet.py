...
def fetch_target_matches():
    ...
    for comp in data.get("competitions", []):
        ...
        for event in comp.get("events", []):
            # 判断比赛开始时间是否在扫描范围内
            event_start = event.get("cutoffTime")
            if event_start and int(event_start) < now:
                continue
            if event_start and int(event_start) > future:
                continue
    ...
    if request.method == 'POST':
        ...
        if event_id:
            ...
            with open(LOG_FILE, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([round_no, match_name, f"BACK @ {price}", amount, result, round(current_balance, 2), notes])
        else:
            # 手动日志录入
            match_name = request.form.get('match')
            ...
            with open(LOG_FILE, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([round_no, match_name, direction, amount, result, round(current_balance, 2), "Manual entry"])
        return redirect('/')
